...
Phi::usage"";
findCompressorChokingBoundary::usage"";
shockLosses::usage"";
Psi::usage"";
PsiIdeal::usage"";
EfficiencyComp::usage"";
etaTotF::usage"";
...
debugAndCleanInterpolationData::usage"";
getDomainBounds::usage"";
...
(* NOZZLE EXPANSION (MOD) *)
	...
	corePoints={PNTa,PNT02,PNT021,PNT03,PNT04,PNT041,PNT05,PNT06,PNT07,PNT9};
...
runTriangleDesign[\[Tau]stage_, DF_, \[Sigma]_, \[Gamma]_, \[Alpha]1deg_]:=Module[
{M1,\[Alpha]2deg,datatriangle},
Run["py"<>" triangle_design_noigv.py "<>ToString[\[Tau]stage]<>" "<>ToString[DF]<>" "<>ToString[\[Sigma]]<>" "<>ToString[\[Gamma]]<>" "<>ToString[\[Alpha]1deg]];
datatriangle=Import["triangledesign.json","JSON"];
M1="Mach1"/. datatriangle;
Print["Mach1 :",M1];
\[Alpha]2deg="alpha2_degrees"/. datatriangle;
...
triangleDesignUpdated[\[Tau]stage_, DF_, \[Sigma]_, \[Gamma]_, \[Alpha]1deg_]:=Module[
...
{M1,\[Alpha]2deg}=runTriangleDesign[\[Tau]stage, DF, \[Sigma], \[Gamma], \[Alpha]1deg];
...
collectCompressorGeometries[allStates_, triangle_, nStages_, massflow_, Rair_] := 
 Module[{CompAreas, rMean, CompMeanDiameter, rTip, rRoot, \[Alpha]1m, 
  ...
  Ncomp = (triangle[[6]]*60) / (2 Pi rMean[[1]]);
...
designFullCompressorUpdated[T0in_,p0in_,\[Beta]c_,\[Eta]stage_,\[Gamma]_,R_,cp_,\[Alpha]indeg_,DF_,\[Sigma]_,desired\[CapitalDelta]T0stage_,massflow_,\[Sigma]material_,\[Rho]blade_,taper_]:=Module[
...
Print["\[Tau]stage,DF,\[Sigma],\[Gamma],\[Alpha]1deg: ", {\[Tau]stage,DF,\[Sigma],\[Gamma],\[Alpha]1deg}];
{{\[Alpha]1deg,\[Alpha]2deg,\[Beta]1deg,\[Beta]2deg},{Ca,U,Cu1,Cu2,Wu1,Wu2},M1,plot}=triangleDesignUpdated[\[Tau]stage,DF,\[Sigma],\[Gamma],\[Alpha]1deg];
Print["Ca :", Ca];
saveTriangle={\[Alpha]1deg,\[Alpha]2deg,\[Beta]1deg,\[Beta]2deg,C1*Ca,C1*U,C1*Cu1,C1*Cu2,C1*Wu1,C1*Wu2,\[Omega]};
Print["SaveTriangle pt1: {\[Alpha]1deg,\[Alpha]2deg,\[Beta]1deg,\[Beta]2deg,C1*Ca,C1*U,C1*Cu1,C1*Cu2,C1*Wu1,C1*Wu2,\[Omega]} = ", saveTriangle];
...
{CrossSection1,rtip1,rhub1}=annulusDesign[massflow,T1,p1,Ca*C1,\[Gamma],R,rmean];
Print["mred stadio: ", massflow*Sqrt[R*T01]/(p01*CrossSection1)];
...
Print["Database contains: T,T0,p,p0,s,rmean,rtip,rhub,CrossSection,M,Mrel"];
Print["Triangle contains: \[Alpha]1,\[Alpha]2,\[Beta]1,\[Beta]2,Ca,U,Cu1,Cu2,Wu1,Wu2,\[Omega]"];
Print["SaveTriangle pt2: {\[Alpha]1deg,\[Alpha]2deg,\[Beta]1deg,\[Beta]2deg,C1*Ca,C1*U,C1*Cu1,C1*Cu2,C1*Wu1,C1*Wu2,\[Omega]} = ", saveTriangle];
...
{sol,U,T,Ca1,Machd,Mach,phi,nred,mred,choked,a1,inputData, outputData, jsonFileIn, jsonFileOut},

nred=((Ncomp/60) diameter)/Sqrt[Rgas T01]; (*Ncomp \[EGrave] in RPM*)
...
(*inputData = <|"gamma" -> \[Gamma],"alpha1_deg" -> N[alpha1*180/\[Pi]], (* export in gradi *)"mred" -> N[mred]|>;
jsonFileIn = "mach_solver_input.json";
jsonFileOut = "mach_solver_output.json";
Export[jsonFileIn, inputData, "JSON"];
RunProcess[{"cmd", "/C", "py mach_solver.py"}];
outputData = Import[jsonFileOut, "JSON"];
Mach = outputData["Mach"];*)
...
U = Pi (Ncomp/60) diameter; (*Ncomp \[EGrave] in RPM*)
Print["U : ",U];
phi = Ca1/U;
...
(* assegna un valore piccolo a k uguale zero *)
If[k==0, k=10^(-16)];
...
nredtot=(Ncomp/60 d[[1]])/Sqrt[R T01];
...
Print["{Can,Tn,Mn,phin,nredn,mredn}=: ",{Can,Tn,Mn,phin,nredn,mredn}];
If[choked,(*Print["choked at mred = ",mredtot," and nred = ", nredtot];*)chokepoint={mredtot,p0n/p01}; Break[]];
Print["debug input betacn : ", {mredn,nredn,Mn,phid[[i]],alpha1[[i]],beta2[[i]],\[Gamma]c,etaStage}];
...
Print["phin (ricalcolato): ", phin];
delT0n=( betacn^((\[Gamma]c-1)/\[Gamma]c)-1)/EfficiencyComp[phin,phid[[i]],alpha1[[i]],beta2[[i]],etaStage];
Print["delT0n con Efficiency: " , delT0n];
Print["delT0n senza EfficiencyComp: " , betacn^((\[Gamma]c-1)/\[Gamma]c)-1];
T0n=T0n(1.+delT0n);
Print["T0n: ", T0n];
p0n=p0n * betacn ;
...
mapPlot,etaAll1Clean,betaAllClean
},
...
{choked,{Candum,Tndum,Mndum,phindum,nreddum,mreddum}}=Block[{$Output={}},StageReducedProperties[mssflow,Ngiri,meanDiameter[[1]],Area[[1]],\[Alpha]1m[[1]],p01,T01,Rgas,\[Gamma]]];

If[choked,mflowmax=mssflow-0.5;Break[];],
{i,0,100,0.1}];
mflowmax=mssflow;
Print["First stage choking occurs at massflow = ", mflowmax];
...
Do[
Print["\[RightArrow] mflow = ", mflow];
Do[
Print["   \:21aa rng = ", rng];
{betaCmap ,etaCmap ,nredmap ,delT0map ,Machmap ,lastStagemap ,etaCmap1 ,perfo, chokepoint }=Block[{$Output={}},multistageCompressorState[
...
(*betaAllClean = debugAndCleanInterpolationData[betaAll, "betaAll"];
etaAll1Clean = debugAndCleanInterpolationData[etaAll1, "etaAll1"];

betaF = Interpolation[betaAllClean, InterpolationOrder -> 6];
etaCF = Interpolation[etaAll1Clean];*)
...
debugAndCleanInterpolationData[data_List, name_String] := Module[
  {len, sample, dups, cleanData},
  
  len = Length[data];
  sample = Take[data, Min[5, len]];
  Print[name <> " length: ", len];
  Print[name <> " sample: ", sample];
  
  dups = Select[Tally[data[[All, 1]]], #[[2]] > 1 &];
  If[Length[dups] > 0,
    Print["Warning: duplicate x-coordinates detected in ", name, ": ", dups];
    cleanData = DeleteDuplicatesBy[data, First];
    Print["Duplicates removed. New length: ", Length[cleanData]];
    cleanData,
...
{x,y,plStab,stabBnd,sol,xopt,yopt,point,stabBndF,stabBndClean,stabBndSorted},
plStab={};
stabBnd={};
xopt=xmin;
Do[
(*sol=FindMaximum[betaF[x,y],{x,0.5(xmin+ xmax),xmin, xmax}];*)
Print["yVal = ", yVal, " numeric? ", NumericQ[yVal]];
sol=FindMaximum[Evaluate[betaF[x,yVal]],{x,xopt,xmin, xmax}];
xopt=x/.sol[[2]];
yopt=sol[[1]];
AppendTo[stabBnd,{xopt,yopt}];
point=plotPoint[xopt,yopt,color] ;
AppendTo[plStab,point],
{yVal,ymin,ymax,(ymax-ymin)/nrng}
];
(*stabBndClean = debugAndCleanInterpolationData[stabBnd, "stabBnd"];
stabBndF = Interpolation[stabBndClean];*)
stabBndSorted = SortBy[stabBnd, First];
stabBndF = Interpolation[stabBndSorted];
...
findCompressorChokingBoundary[chokeList_]:=Module[{sortedList,boundaryList,interpBoundary,interpBoundaryCleanData},

sortedList=GatherBy[Sort[chokeList,#1[[1]]<#2[[1]]&],First];
boundaryList=Table[sortedList[[i,1]],{i,1,Length[sortedList]}];
(*interpBoundaryCleanData = debugAndCleanInterpolationData[boundaryList, "boundaryList"];
interpBoundary=Interpolation[interpBoundaryCleanData,InterpolationOrder->2];*)
...
Plot[stabBndF[x],{x,stabBndF["Domain"][[1,1]],stabBndF["Domain"][[1,2]]},Filling->Top,FillingStyle->Directive[Opacity[1],White]],
plStab,
PlotRange->{{lowerLimit,xmax},{1,upperlimit}},
Frame->True,Axes->False,FrameLabel->{"\!\(\*SubscriptBox[\(m\), \(red\)]\)","\[CapitalPi]"}];

Return[mapPlot]];


getDomainBounds[ifun_InterpolatingFunction] := ifun["Domain"][[1]]
