...
def off_design_compressor_map(
    ...
    # Find stability boundary (max pi_c for each nred, i.e., along rows of Pi_grid)
    stability_boundary = np.nanmax(Pi_grid, axis=1)  # shape (n_mred,)

    # Broadcast the stability boundary to all rows for each mred (column)
    mask = Pi_grid > stability_boundary[:,None]  # shape (n_nred, n_mred)
    # Set points above the boundary to NaN (they won't be plotted)
    ...
    contour_N = plt.contour(M, Pi_grid_masked, N, colors='white', linewidths=1, linestyles='dotted')
    plt.clabel(contour_N, fmt="N=%.2f", colors='black', fontsize=8)
    plt.colorbar(cp, label='$N_{red}$')
    plt.plot(mred_grid, stability_boundary, 'r--', label='Stability Boundary',linewidth=1.5)
   ...
   #with open('compressor_map_data.json', 'w') as f:
    ...
    return results, choked_points, Pi_grid, stability_boundary, Pi_grid_masked
...
def main():
    ...
    Nshaft_grid = np.linspace(0.2*NcompOnD, 1.5*NcompOnD, nrng)   # Example range, adjust as needed
