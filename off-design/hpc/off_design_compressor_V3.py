...
def main():
    massflow = 114.447  # kg/s
    Ncomp = 13832.629629250854  # RPM
    T02OnD = 565.868 # K
    p02OnD = 372858 # Pa
    phim = [1.0695] * 6
    gamma = 1.4
    ...
    beta2m = [0.0] * 6
    CompAreas = [
        0.245039, 0.205379, 0.174464, 0.14991, 0.130113, 0.113919
    ]
    CompMeanDiameter = [0.295497] * 6
    etaStage = 0.9  # Replace with your actual value for ηpc

    # Call the principal function
    results = multistageCompressorState(
        T02OnD, p02OnD, massflow, Ncomp, phim, Rair, gamma, alpha1m, beta2m, CompAreas, CompMeanDiameter, etaStage
    )
    ...
    mred_des=0.50406  # Reduced mass flow at design point;
    Pi_des=3.47813  # Pressure ratio at design point
    NcompOnD = Ncomp # RPM
    massflowOnD = massflow # kg/s

    mapResolution = 256  # or 256 for higher accuracy
    nrng = mapResolution
    nmflow = mapResolution

    mflowmax = find_max_massflow_first_stage_choke(
    massflowOnD, NcompOnD, CompMeanDiameter[0], CompAreas[0], alpha1m[0], p02OnD, T02OnD, Rair, gamma
    )
    ...
    results = off_design_compressor_map(
         T02OnD,p02OnD, massflow_grid, Nshaft_grid, alpha1m, beta2m, CompAreas, CompMeanDiameter, phim, etaStage, gamma, Rair,
