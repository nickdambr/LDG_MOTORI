...
from component_maps import get_lpc_map_data, get_hpc_map_data, get_inlet_map_data, get_nozzle_mdot_red

def get_freestream_mass_flow_param(M0, gamma=1.4):
    """
    Calculates the reduced mass flow parameter for the freestream at a given Mach number.
    mdot_red = M * (1 + ((gamma - 1)/2) * M^2) ** (-(gamma + 1)/(2*(gamma - 1)))
    """
    return M0 * (1 + ((gamma - 1)/2) * M0**2) ** (-(gamma + 1)/(2*(gamma - 1)))
...
class TurboFan(EquationSystem):
    ...
    def get_equations(self, var_values):
        ...
        eta_LPC = self.params['eta_LPC']
        eta_HPC = self.params['eta_HPC']
        eta_LPT = self.params['eta_LPT']
        eta_HPT = self.params['eta_HPT']
        M0 = self.params['M0']
        ...
        # --- Call Component Maps ---
        # The map functions now return mdot_red directly
        mdot_red_LPC = get_lpc_map_data(Pi_LPC, N_red_LPC)
        mdot_red_HPC = get_hpc_map_data(Pi_HPC, N_red_HPC)
        inlet_data = get_inlet_map_data(M0)
        ...
        return [eq1, eq2, eq3, eq4, eq6, eq7, eq8, eq9, eq10]
